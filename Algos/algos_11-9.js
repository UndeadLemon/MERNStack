...
*/
   ...
   * - Space O(?).
   */
  const coronaVirusAtRiskFunctional = (persons, atRisk =[]) => {persons.forEach(person => { !person.isSocialDistancing && person.friends.reduce((previous, current, index) => (!person.friends[index].isSocialDistancing && !person.friends[index].hasCovid) || !previous ? false : true ) ? atRisk.push(person.firstName + ' ' + person.lastName): null})
  return atRisk
}
